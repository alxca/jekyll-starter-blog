(function($) {
  $.fn.searchPosts = function(options, data) {
    let settings = $.extend({
      ...
      template: '<li><article><a class="document-load" href="{url}" onclick="documentLoad(event, this)">{title} <span class="entry-date"><time datetime="{date}">{date}</time></span></a></article></li>',
